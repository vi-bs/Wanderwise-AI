...
Wanderwise AI follows a modern full-stack TypeScript architecture, separating the frontend presentation layer from the AI backend logic.

-   **Frontend**: A [Next.js](https://nextjs.org/) application using the App Router. It's responsible for all UI rendering, user input, and client-side state management.
-   **AI Backend**: An AI orchestration layer built with [Genkit](https://firebase.google.com/docs/genkit). It defines the logic for calling large language models (LLMs) like Gemini, structures the input and output, and can be extended to include other tools or services.
-   **Communication**: The Next.js frontend calls the Genkit flows, which are exposed as serverless functions. This communication happens via standard client-server HTTP requests.
...
-   **`localStorage`**: Used as a simple, client-side "database" to pass large JSON payloads between pages without complex state management libraries.
...
2.  **Submit**: On submit, `handleFormSubmit` is called. It stringifies the form data and saves it to `localStorage`.
